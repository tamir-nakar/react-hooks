...
## 📝 Your Notes
When passing the `useState`method a function as an initializer, That function
will only run once(!) on load. It's a good practice to use
that when we don't want to make heavy comptations each render
when we need the value only on startup 
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=02%3A%20useEffect%3A%20persistent%20state&em=nakar.tamir%40gmail.comnakar.tamir%40gmail.com).
